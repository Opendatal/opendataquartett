...
(function($) {
  ...
  $.fn.toptrumps = function(options) {
    var methods = {
      ...
      'renderCards' : function(settings, tt) {
        ...
        if (tt.topTrumpsTurn === 'user') {
          $('.tt-card-field', tt).click(function() {
            ...
            var id = ident_arr[(ident_arr.length - 1)];

            //methods.compareCards(ident_arr[(ident_arr.length - 1)], tt.topTrumps, tt);
            //$('.tt-cpu',tt).remove();
            $('.tt-card', tt).remove();

            methods.renderCard(cards.cpu, 'cpu', settings, tt, true);
            methods.renderCard(cards.user, 'user', settings, tt);
            ...
            $('#tt-card-field-cpu-' + id).addClass('active');

            $('a.tt-next-button', tt).click(function(e) {
              $('a.tt-next-button', tt).remove();
              methods.compareCards(id, tt.topTrumps, tt);
            });
          ...
          $('#tt-card-field-user-' + field).addClass('active');
      ...
      'renderCard' : function(card, player, settings, tt, force ) {
        var $card = $('<div/>').addClass('tt-card');

        if (player === tt.topTrumpsTurn || player === 'user' || force) {
      ...
      'renderCardFields' : function(card, fields, trumps, player) {
        ...
        for (var i = 0; i < fields.length; i++) {
          ...
          var value = card.fields[i];

          if (typeof fields[i].unit !== 'undefined') {
            value += " " + fields[i].unit;
          }
          $('<span/>').addClass('tt-card-field-value').html(value).appendTo($field);
